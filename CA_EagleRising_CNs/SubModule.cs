                ...
                allPolicies.ForEach(policyObject =>
                {
                    switch (policyObject.StringId)
                    {
                        case "_publicLandCommissions":
                            policyObject.Initialize(new TextObject("{=TmiRL2YV}Public Land Commissions"),
                                new TextObject("{=h1clv2g8}Land commissioners buy uncultivated land from rich land owners and redistribute to poor landless citizens and veterans to cultivate"),
                                new TextObject("{=HVdMluuL}create a public land commission"),
                                new TextObject("{=DWvCZUfx}+ 0.8 hearth growth in villages\n-100 gold per village\n-0.1 influence per rural notable unless relations at least 30"),
                                -0.4f, 0.4f, 0.4f);
                            break;
